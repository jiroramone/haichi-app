...
def render_horse_cards_carousel(h_list, selected_venue, curr_df, cards_per_row=1, block_key=None):
    ...
    if total_pages > 1:
        ...
        with nav_cols[0]:
            st.button("◀", key=f"prev_{block_key}", disabled=(current_page == 0),
                      on_click=go_prev, use_container_width=True)
        with nav_cols[1]:
            st.markdown(
                f"<div style='text-align:center;color:#666;font-size:12px;padding-top:8px;'>"
                f"{start_idx+1}〜{end_idx}/{total}頭 ({current_page+1}/{total_pages}p)</div>",
                unsafe_allow_html=True)
        with nav_cols[2]:
            st.button("▶", key=f"next_{block_key}", disabled=(current_page >= total_pages - 1),
